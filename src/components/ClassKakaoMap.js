    ...
    getData() {
        ...
        fetch(url, { method: 'get' }) //체인방식으로 실행. 장점은 줄 순서대로 각각 실행 결과가 마무리 된 후 다음 줄이 실행 된다.
            ...
            .then(contents => { //json으로 변환된 응답데이터인 contents 를 가지고 구현하는 내용
                    ...
                    positions.push(
                        {
                            ...
                            latlng: new kakao.maps.LatLng(element["lat"]['_text'], element["longi"]['_text']) // 위도(latitude),
                ...
                console.log(jsonData["item"][index]["lat"]['_text'], jsonData["item"][index]["longi"]['_text']);
            ...
            .catch((err) => console.log('에러: ' + err + '때문에 접속할 수 없습니다.'));// then(content) 끝 //.then함수 끝 추가. 위 기존 코드 중략부분 중 positions 변수 부분 지운다.
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Link to="/"><button className="form-control btnbtn-primary" id="btnHome">홈으로</button></Link>
